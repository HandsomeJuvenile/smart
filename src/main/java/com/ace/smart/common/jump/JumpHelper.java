package com.ace.smart.common.jump;
    ...
    public int[] getHalmaAndBoardXYValue(File currentImage) throws IOException {
        ...
        for (int y = gameScoreBottomY; y < height; y++) {
            for (int x = 0; x < width; x++) {
                processRGBInfo(bufferedImage, x, y); // 得到每一个像素的 rgb的值
        ...
        if (halmaXSum != 0 && halmaXCount != 0) {
            ...
            //上移棋子底盘高度的一半
            int halmaY = halmaYMax - halfBaseBoardHeight;// 上移一半之后差不多就是在这个棋子的中心点坐标
            //从gameScoreBottomY开始
            for (int y = gameScoreBottomY; y < height; y++) {// 从y坐标开始查找每个像素
                ...
                // 第二个跳板的位置的y坐标肯定要比棋子的y坐标要高，那么只要一旦找到色差 获得多个点 就可以求取平均值 得到x的坐标位置
                if (boardX > 0) {
                    break;
                }
                int boardXSum = 0;
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    //从上往下逐行扫描至下一个跳板的顶点位置，下个跳板可能为圆形，也可能为方框，取多个点，求平均值  扫描的rgb与周围rgb颜色超过10
                    // 根据色差比较
            ...
            /**
             * 跳板的位置每次角度都是一致的，所以直接假设两个跳板的中心点 求得到该斜率
             * Math.abs(boardY1 - boardY2) / Math.abs(boardX1 - boardX2)  就是两个跳板的斜率
             * 再去乘上Math.abs(boardX - halmaX)  就可以得到第二个跳板的中心坐标
             *像素的y轴是不断减少的，所以再去通过棋子的y轴去减去就可以得到第二个跳板的y轴坐标
             */
            boardY = (int) (halmaY - Math.abs(boardX - halmaX) * Math.abs(boardY1 - boardY2)
                    / Math.abs(boardX1 - boardX2));

            System.out.println(Math.abs(boardX - halmaX) * Math.abs(boardY1 - boardY2)/ Math.abs(boardX1 - boardX2));
